...
import sample.{Device, DeviceGroup, DeviceGroupQuery}
...
/**
  ...
  *
  */
class DeviceGroupQuerySpec extends TestKit(ActorSystem("deviceGroupQuerySpec")) with ImplicitSender with WordSpecLike with Matchers with BeforeAndAfterAll {
  ...
  "DeviceGroupQuery" must {
    "return temperature value for working devices" in {
      ...
      val queryActor = system.actorOf(DeviceGroupQuery.props(actorToDeviceId = Map(device1.ref -> "device1", device2.ref -> "device2"),
        requestId = 1,
      ...
      queryActor.tell(Device.RespondTemperature(requestId = 0, Some(1.0): Some[Double]), device1.ref)
      queryActor.tell(Device.RespondTemperature(requestId = 0, Some(2.0): Some[Double]), device2.ref)
